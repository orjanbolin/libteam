...
static int lw_ttdp_sock_open(struct lw_ttdp_port_priv* ttdp_ppriv) {
	...
	/* This is the one used for SENDING. */
	err = teamd_packet_sock_open_type_ext(SOCK_RAW | SOCK_NONBLOCK, &ttdp_ppriv->out_sock,
					  ttdp_ppriv->start.common.tdport->ifindex,
					  htons(ETH_P_8021Q), NULL, NULL);
	teamd_ttdp_log_dbgx(ttdp_ppriv, "Socket (write) open result %d", err);

	/* This is the socket used for READING. It has to be left as ETH_P_ALL. */
	err = teamd_packet_sock_open_type(SOCK_RAW, &ttdp_ppriv->start.psr.sock,
					  ttdp_ppriv->start.common.tdport->ifindex,
					  htons(ETH_P_ALL), &ttdp_hello_fprog, NULL);
	teamd_ttdp_log_dbgx(ttdp_ppriv, "Socket (read) open result %d", err);
	...
	int priority = 7;
	if (setsockopt(ttdp_ppriv->out_sock, SOL_SOCKET, SO_PRIORITY, &priority, sizeof(priority)) != 0) {
...
static void lw_ttdp_sock_close(struct lw_ttdp_port_priv *ttdp_ppriv)
{
	...
	close(ttdp_ppriv->out_sock);
}

static int __get_port_curr_hwaddr_out(struct lw_ttdp_port_priv *priv,
				  struct sockaddr_ll *addr, size_t expected_len)
{
	...
	err = teamd_getsockname_hwaddr(priv->out_sock, addr, expected_len);
	if (err)
		return err;
...
static void construct_default_frame(struct ab* parent, struct lw_ttdp_port_priv *ttdp_ppriv,
	...
	if (__get_port_curr_hwaddr_out(ttdp_ppriv, &source_addr, 6) != 0) {
		/* FIXME ERR */
...
static void lw_ttdp_port_removed(struct teamd_context *ctx,
	...
	/* In case teamd_ttdp_log_dbgx is not defined: */
	(void)ttdp_ppriv;
