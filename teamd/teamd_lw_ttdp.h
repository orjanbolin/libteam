...
struct ab {
	...
	struct teamd_workq end_agg_detection_workq;

	int parasite_sockfd;

	/* Number of TOPOLOGY frames heard being sent through this aggregate */
	uint32_t topo_ctr_out;
	/* Number of inbound TOPOLOGY frames heard during the last end agg detection interval.
	   This is then reset. */
	uint32_t topo_ctr_in;
	/* As above, but never reset */
	uint32_t topo_ctr_in_total;
	bool is_topo_ctr_in_increasing;
